...
inject_secrets() {
  ...
  for secrets_file in "${secret_files[@]}"; do
    ...
    # Pattern matches: VAR="value", VAR='value', VAR='has'\''quote', VAR=unquoted
    # Escape group (\\\\''[^']*')* matches: backslash, two quotes, content, quote
    local safe_pattern="^([A-Za-z_][A-Za-z0-9_]*=(\"[^\"]*\"|'[^']*'(\\\\''[^']*')*|[^;\\\`\$()&<>|[:space:]]+)|#.*|[[:space:]]*)$"
    if grep -vE "${safe_pattern}" "${resolved_file}" >/dev/null 2>&1; then
      ...
      # Log line numbers only to avoid leaking secret values
      local unsafe_count
      unsafe_count="$(grep -cvE "${safe_pattern}" "${resolved_file}" 2>/dev/null)" || unsafe_count=0
      debug_log "Found ${unsafe_count} unsafe line(s) - check source file format"
      rm -rf "${temp_dir}"
      return 1
    fi

    # Source the resolved env file with proper variable export
    # Note: SC1090 disabled because resolved_file is dynamically generated and
    # validated above (safe_pattern check ensures only variable assignments)
    set -a
